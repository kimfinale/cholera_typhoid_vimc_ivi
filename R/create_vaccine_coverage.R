...
create_vaccine_coverage <- function(country = NULL,
                                    population = NULL,
  ...
  if ("campaign" %in% dat$activity_type) {
    ...
    for (j in seq_along(yr)) {
      ...
      cover_adj <- cover * as.double(cmp_yr$target) /
        sum(population[age_index_start:age_index_stop, yr[j]])

      if (cover_adj > 1) {
        message("vaccine coverage is larger than 1")
      }
      vacc_coverage[age_index_start:age_index_stop, yr[j]] <- min(1, cover_adj)
    }
  }
