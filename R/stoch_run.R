...
stoch_run <- function(disease = NULL,
                      country = NULL,
                      vacc_scenario = NULL,
                      runs = 30,
                      year = 2000:2100,
  ...
  cntry <- country
  rm(country)
  stoch_param <- data.frame( run_id = rep(1:runs, length(cntry)),
                             country = unlist(lapply(cntry, function(x) rep(x, runs))), ir = NA)
  index <- 1

  cat( "country =", cntry, "\n")
  mean_ir <- incidence_rate[tolower(disease) == tolower(dis) & country == cntry, incidence_rate_100Kpyo]
  ...
  if (tolower(vacc_scenario) == "routine"){
    ...
    vp <- calculate_vaccine_protected(disease = dis, country = cntry, population = pop, vacc_coverage = vc)
    pop <- pop - vp
  }
  ...
  cohort_size <- setup_cohorts(country = cntry, rel_risk_low = 1.0) # cohort size is assumed to be the total population
  ...
  for (i in seq_len(runs)) {
    ...
    stoch_param$ir[index] <- irs[i] # record keeping for parameters that are varied across runs

    index <- index + 1
    cases <- calculate_cases(disease = dis, population = pop, country = cntry, ir = irs[i])
    cases_list[[i]] <- cases
    deaths_list[[i]] <- cases * cfr
    dalys_list[[i]] <- calculate_DALY(disease = dis, cases = cases, country = cntry)
  ...
  saveRDS(stoch_output, paste0("data/stoch_output_", dis, "_", cntry, "_", vacc_scenario, ".rds"))

  cohort_size <- vimc_report(disease = dis, country = cntry, sim_output = cohort_size, value_name = "cohort_size")
  cases <- vimc_report(disease = dis, country = cntry, sim_output = cases_list, value_name = "cases")
  deaths <- vimc_report(disease = dis, country = cntry, sim_output = deaths_list,value_name = "deaths")
  dalys <- vimc_report(disease = dis, country = cntry, sim_output = dalys_list, value_name = "dalys")
  ...
  saveRDS(report, paste0("data/report_", dis, "_",  cntry, "_", vacc_scenario,  ".rds"))
