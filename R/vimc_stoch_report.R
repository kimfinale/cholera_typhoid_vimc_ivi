...
vimc_stoch_report <- function (
  ...
  age = 0:100,
  run_ids = NULL,
  column_order = c("disease", "run_id", "year", "age", "country", "country_name",
  ...
  for(i in 1:nvar){
    ...
    # for(j in seq_len(runs)){
      ...
      # df$run_id <- j
      df$run_id <- run_ids[j]
      df_long <- tidyr::pivot_longer(df, cols = as.character(year),
                                     names_to = "year",
                                     values_to = sim_output$var[i])
  ...
  df2 <- list2[[1]]
  for(k in 2:nvar){
    df2 <- left_join(df2, list2[[k]],
                     by = c("disease", "run_id", "year", "age", "country",
                                             "country_name"))
  }
