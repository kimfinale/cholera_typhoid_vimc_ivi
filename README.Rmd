...
for(dis in disease) {
  for (vacc in vacc_scenario) {
    if (!(dis == "Cholera" & vs == "routine")) {
      fls <- list.files("data", pattern = paste0("^rep.*", dis, ".*", vacc, ".*rds$"), full.names = T)
      fls
      x <- lapply(fls, function(x) readRDS(x))
      res <- do.call(rbind, x)
      saveRDS(res, paste0("data/central_", dis, "_", vacc, ".rds"))
    }
  }
}
...
vacc <- "campaign"
cntry <- "Somalia"
res <- readRDS(paste0("data/central_",  dis, "_", vacc, ".rds"))
  ...
  for (vs in vacc_scenario) {
    if (!(d == "Cholera" & vs == "routine")) {
      ...
      for(i in seq_along(cntry)){
        output1 <- readRDS(paste0("data/stoch_output_", d, "_",  cntry[i], "_", vs, ".rds"))
        list1[[i]] <- vimc_stoch_report(disease = d, country = cntry[i], sim_output = output1)
      }
      res <- do.call("rbind", list1)
      saveRDS(res, paste0("data/stoch_", d, "_", vs, ".rds"))
    }
  ...
  for (v in vacc) {
    for (sim in sims){
      if (!(tolower(d) == "cholera" & tolower(v) == "routine" )){
        fls <- list.files("./data", pattern = paste0(sim, "_", d, "_", v, ".rds$"), full.names = T)
        fls
        res <- readRDS(fls)
        res$disease <- d
