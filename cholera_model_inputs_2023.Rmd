---
title: "Cholera model inputs 2023"
...
## Incidence rate by country (detailed preparation process)
...
ir_countries <- data.frame(country = target_countries_cholera)  
...
Wikipedia has a separate [page](https://en.wikipedia.org/wiki/2016%E2%80%932022_Yemen_cholera_outbreak) for the cholera outbreak in Yemen. [EMRO WHO](https://www.emro.who.int/health-topics/cholera-outbreak/latest-updates.html) has recent data 

Monthly report appears at the [page](https://www.emro.who.int/health-topics/cholera-outbreak/latest-updates.html)
...
We took the average of the last 4 years, which is an arbitrary point. However, it appears that the outbreak is going down and therefore we did not include the year 2017, peak of the outbreak. 2018-2021,
...
library(dplyr)
yr = c(2010,2011,2016,2017,2018,2019,2020,2021)
ann_case = c(300,31789,15751,1032481,371326,861096,275712,72765)

pop = population_data |> filter(country == "Yemen") |>
  select(as.character(yr)) |> colSums()

# we assume that the cases were not reported rather than zero, in the missing years. 

overall_ir = sum(ann_case) / sum(pop) * 100000
...
(overall_ir = sum(haiti_df$num_cholera_reported) / sum(pop) * 100000)
...
```{r}
total_cases = 28100000
cntry = "Bangladesh"
year = 2015
bangla <- data.frame(country = cntry, 
                       year = year, 
                       num_cholera_reported = c(total_cases))

pop = population_data |> filter(country == cntry) |>
  select(as.character(years)) |> colSums()

(overall_ir = sum(total_cases) / sum(pop) * 100000)

reported_cholera <- rbind(reported_cholera, bangla)
```
...
74 confirmed cases as of 30 August 2018 [link](https://www.ecdc.europa.eu/sites/default/files/documents/cholera-algeria-rapid-risk-assessment-september-2018.pdf)
From 7 August to 6 September, 217 cases with cholera-like symptoms 
[link](https://www.who.int/emergencies/disease-outbreak-news/item/14-september-2018-cholera-algeria-en)
...
# only 2017 - 2021 data will be used
reported_cholera[country == "Haiti" & year %in% 2017:2023] -> hai
mean <- mean(hai$reported_case_per_100K, na.rm = T)
median = median(hai$reported_case_per_100K, na.rm = T)
sd = sd(hai$reported_case_per_100K, na.rm = T)
sample_size = sum(!is.na(hai$reported_case_per_100K))
...
ir = as.data.frame(ir)
# the following does not seem to be a valid syntax for data.table why?
ir[ir$country == "Haiti", c("mean", "median", 
"sd", "sample_size", "max", "min", "se", "upper", "lower")] = vec
...
# ir_rst_WHO_JHU = as.data.frame(ir_rst_WHO_JHU)
ir_rst_WHO_JHU %>% 
  mutate(ir_sim = ifelse((is.na(raster_estimate) | raster_estimate == 0),
                         mean, raster_estimate), 
         ir_sim_sd = 1.5 * ir_sim) %>%
  dplyr::select(country, ir_sim, ir_sim_sd) -> overall_incid_rate_cholera
...
# irs
# mean(irs, na.rm = T)
# 17803.97 infections per year (Azman et al. 2020) and 1 in 10 is assumed to be asymptomatic. The estimates for the ratio of asymptomatic to symptomatic infections have ranged from 3 to 100

overall_incid_rate_cholera[country == "Bangladesh", ir_sim := 17803.97/10]
overall_incid_rate_cholera[country == "Bangladesh", ir_sim_sd := 17803.97/10*1.5]

overall_incid_rate_cholera[country == "India", ir_sim := 81.546]
overall_incid_rate_cholera[country == "India", ir_sim_sd := 81.546*1.5]
overall_incid_rate_cholera[country == "Yemen", ir_sim := 1122.258]
overall_incid_rate_cholera[country == "Yemen", ir_sim_sd := 1122.258*1.5]
overall_incid_rate_cholera[country == "Haiti", ir_sim := 589.9052]
overall_incid_rate_cholera[country == "Haiti", ir_sim_sd := 589.9052*1.5]
