---
title: "Cholera model inputs updates 2025"
...
Model inputs associated with population and life expectancy are defined in `typhoid_model_inputs.Rmd`
```{r eval=FALSE}
# templates <- fread("inst/extdata/central-burden-template.202310gavi-4.Cholera_IVI-Kim_standard.csv")
...
# 
# ocv1_cov <-
#   fread("inst/extdata/coverage_202310gavi-4_cholera-ocv1-default.csv")
# 
# ocv12_cov <-
#   fread("inst/extdata/coverage_202310gavi-4_cholera-ocv1-ocv2-default.csv")
# 
# # standardize country names
# ocv1_cov$country <- clean_country_names(ocv1_cov$country)
# # standardize country names
# ocv12_cov$country <- clean_country_names(ocv12_cov$country)
# 
# vacc_cov_input_cholera <- list()
# vacc_cov_input_cholera[[1]] <- ocv1_cov
# vacc_cov_input_cholera[[2]] <- ocv12_cov
# usethis::use_data(vacc_cov_input_cholera, overwrite = T)
# 
# vacc_scenarios_cholera <- 
#   c("novacc", "campaign_default_ocv1","campaign_default_ocv12")
# usethis::use_data(vacc_scenarios_cholera, overwrite = T)

# 
# dis <- "Cholera"
# parameters <- data.table::fread("inst/extdata/parameters.csv")
# # usethis::use_data(parameters, overwrite = T)
# parameter_data_cholera <-
#   parameters %>% filter(tolower(disease) == tolower(dis))
...
# population data for the 2023 runs were created in the # typhoid_model_inputs_2023.Rmd and used here
# pop <- fread("inst/extdata/202310gavi-4_dds-202208_int_pop_both.csv")
# pop %>% select(country, age_from, as.character(2000:2100)) -> pop
```

## OCV indirect effectiveness data
...
ive_dat <- data.frame(eff_vacc_cov = c(eff_vc_kolkata, eff_vc_matlab), indirect_vacc_eff = c(ive_kolkata, ive_matlab))
...
## Incidence rate data (per 100,000 person-year)

```{r}
person_years <- 100000
overall_incid_rate_cholera_2023 <- readRDS("outputs/overall_incid_rate_cholera_2023_20250703.rds")
ir_rst_WHO_JHU <- data.table::fread("outputs/ir_cholera_raster_WHO_JHU.csv")
```

```{r}
cntry_update <- c("Yemen", "Haiti", "India", "Bangladesh")
```

### Incidence rate by country 

Cholera-country level incidence rates are based on the study by Lessler et al. (2018) in which the grid-level incidence rate (20 km by 20 km) are provided. On the web repository (http://www.iddynamics.jhsph.edu/projects/cholera-dynamics), estimates for the number of cases ("case.tif") and incidence rates ("rate.tif") were downloaded.

The raster-based incidence rates were compared with the WHO case reports, in particular, more recent reports (since 2000).
...
af <- readRDS("data/africa_adm0_shp.rds")
# country boundary to identify grid cells that belong to a country
...
df$rate <- df$cases / df$population * person_years
...
# saveRDS(incidence_rate_cholera, sprintf("outputs/cholera_incidence_rate_country_pop_2010_%s.rds", tstamp()))

# usethis::use_data(incidence_rate_cholera, overwrite = T)

## Prepare 2010 population raster
# ppp <- raster("data/ppp_2010_1km_Aggregated.tif")
# rst <- raster::aggregate(ppp, fact = 20, fun = sum)
# rst <- raster::crop(rst, extent(af), snap = "out")
# rst <- mask(rst, mask = af)
# names(rst) <- "ppp_2010_20km_aggregated"
# # saveRDS(rst, sprintf("outputs/ppp_af_2010_20km_%s.rds", tstamp()))
# file moved from the outputs to the data folder
# rst <- readRDS("data/ppp_af_2010_20km_20231121.rds")
# raster::plot(rst)
```


WHO incidence data can be acquired through the [Global Health Observatory data repository](https://apps.who.int/gho/data/?theme=main)

```{r}
reported_cholera <- fread("inst/extdata/CHOLERA_case_21Noc2023.csv")
...
reported_cholera <- as.data.frame(reported_cholera)
```

We identified the country-specific incidence rates for the countries where outbreaks have been reported. These countries included Yemen (Eastern Mediterranean Region), Haiti (Americas Region), Bangladesh (South-East Asian Region), and India (South-East Asian Region).

#### Yemen 

[Our World In Data](https://ourworldindata.org/country/yemen#search=cholera) has the cholera incidence, deaths, and case fatality ratio data. As of 23 November 2023, the following data are available for Yemen:
...
------------
 ...
 - Monthly reports appear at the [page](https://www.emro.who.int/health-topics/cholera-outbreak/latest-updates.html)

 - The [latest report](https://www.emro.who.int/pandemic-epidemic-diseases/cholera/acute-watery-diarrhoeacholera-updates-15-october-2023.html) was on Oct 15, saying 5623 cases and 7 deaths for January-September 2023. 

 - Between 16 October and 31 December 2023, a total of 1,018 reported cases were associated with the outbreak, spanning 23 districts in 9 governorates of southern and eastern Yemen.(https://www.unocha.org/publications/report/yemen/yemen-humanitarian-update-issue-11-december-2023)
 - 3279 cases in 2023, which did not account for the data from Northern Yemen (https://www.emro.who.int/pandemic-epidemic-diseases/cholera/acute-watery-diarrhoeacholera-updates-31-december-2023.html)
 
 - 249 900 suspected cases of cholera, with 861 associated deaths (Jan 1 - Dec 1, 2024) from WHO [page](https://www.emro.who.int/media/news/yemen-reports-the-highest-burden-of-cholera-globally.html) 
```{r}
# EDIT: 11 Jan 2024
# cases are adopted from Our World in Data since 2010
library(dplyr)
yrs = c(2010,2011,2016,2017,2018,2019,2020,2021,2023,2024)
ann_case = c(300,31789,15751,1032481,371326,861096,275712,72765,3279,249900)

yrs = yrs[1:8]

pop = population_data |> filter(country == "Yemen") |>
  select(as.character(yrs)) |> colSums()

# we assume that the cases were not reported rather than zero, in the missing years. 
overall_ir = sum(ann_case) / sum(pop) * person_years

# The above value is also suppported by the [reliefweb](https://reliefweb.int/report/yemen/yemen-cholera-outbreak-dg-echo-dg-echo-partners-echo-daily-flash-4-december-2023#:~:text=The%20cumulative%20number%20of%20cholera,000%2C%20including%204%2C000%20related%20deaths)
# years = 2016:2021
# cases = 2500000
# pop = population_data |> filter(country == "Yemen") |>
#   select(as.character(years)) |> colSums()
# overall_ir = sum(cases) / sum(pop) * 100000

yemen <- data.frame(country = "Yemen",
                    year = yr,
                    num_cholera_reported = ann_case)

reported_cholera <- rbind(reported_cholera, yemen)

# 1:8 was used just to reproduce the burden when the data up to 2021 
# was available

xbar <- mean(ann_case[1:8] / pop * person_years)
sx <- sd(ann_case[1:8] / pop * person_years)

overall_incid_rate_cholera[country == "Yemen", ir_sim := xbar]
overall_incid_rate_cholera[country == "Yemen", ir_sim_sd := sx]
```
...
Situation report is available from the Ministry of Health (MINISTÈRE DE LA SANTÉ PUBLIQUE ET DE LA POPULATION(MSPP)) [hompeage](https://mspp.gouv.ht/site/downloads/Sitrep%20cholera_07_novembre_2023.pdf)

Haiti has been free of cholera since January 2019 [Wikipedia page](https://en.wikipedia.org/wiki/2010s_Haiti_choleraoutbreak).

The new first case reappeared in Oct 1, 2022 [link](https://mspp.gouv.ht/site/downloads/Cholera%20communique%201%20.pdf) and 
69,992 cumulative suspected cases as of Nov 6. 2023 [link](https://mspp.gouv.ht/site/downloads/Sitrep%20cholera_07_novembre_2023.pdf). 

In general documents on disease occurrence area available [here](https://www.mspp.gouv.ht/documentation/).

```{r}
haiti <- fread("inst/extdata/haiti_2010_2023.csv")
haiti_df <- data.frame(country = "Haiti", 
                       year = 2010:2023, 
                       num_cholera_reported =  haiti$cases)

years = 2010:2023
pop = population_data |> filter(country == "Haiti") |>
  select(as.character(years)) |> colSums()

(overall_ir = sum(haiti_df$num_cholera_reported) / sum(pop) * person_years)

ir_data <- haiti_df$num_cholera_reported / pop * person_years
xbar <- mean(ir_data)
sx <- sd(ir_data) 
...
# update the simulation input parameter, overall_incid_rate_cholera

overall_incid_rate_cholera[country == "Haiti", ir_sim := xbar]
overall_incid_rate_cholera[country == "Haiti", ir_sim_sd := sx]
```


#### Bangladesh

[Azman et al.](https://www.thelancet.com/journals/lanmic/article/PIIS2666-5247(20)30141-5/fulltext) estimates seroincidence to be $17.3\% (95\% ~\textrm{CI}~ 10.5-24.1)$.
This refers to the number of infections. 

EDIT 2025-07-02
However, incidence of symptomatic infections for Bangladesh is better described in the study (Hegde et al. (2024) Nat Med. 2024 Mar;30(3):888–95).  

> The incidence rate of symptomatic cholera, including both those who were
medically attended and those who were not, was 0.6 per 1,000 per
year (95% CrI = 0.5–0.8) with elderly people having twice the incidence
rate compared with others (Table 2).


```{r}

# average from 1990 to 2000 as the data from 2001 not available
reported_cholera$year <- as.integer(reported_cholera$year)
...
# medically attended and those who were not, was 0.6 per 1,000 per
# year (95% CrI = 0.5–0.8)
xbar1 <- 0.6 / 1000 * person_years
# upper <- 0.8 / 1000 * person_years
# lower <- 0.5 / 1000 * person_years
# sx <- (((upper - lower)/2)/1.96)^2
```



On the other hand, 
Paul RC, Faruque ASG, Alam M, Iqbal A, Zaman K, Islam N, et al. Incidence of severe diarrhoea due to Vibrio cholerae in the catchment area of six surveillance hospitals in Bangladesh. Epidemiol Infect. 2016 Apr;144(5):927–39. 
```{r}
# Table 3.  
df <- data.frame(population = c(10368, 993, 225, 247, 176, 186)*1e3, 
                 cases = c(16950, 1640, 257, 51, 264, 148),
                 prop_health_seek = c(0.62, 0.33, 1.0, 0.70, 0.40, 0.44),
                 ir_adj = c(2.6, 4.9, 1.1, 0.3, 3.7, 1.8))
ir_adj_test <- df$cases / df$population / df$prop_health_seek * 1e3
ir_adj_test - df$ir_adj

ir_weighted <- weighted.mean(df$ir_adj, df$population)

# Adjust for health seeking behavior
df$cases_adj <- df$cases / df$prop_health_seek


# Total estimated cases and population
total_cases <- sum(df$cases_adj)
total_pop <- sum(df$population)

# Incidence rate per 100,000
inc_rate <- (total_cases / total_pop) * person_years
# factor of 2 is multiplied to account for a conservative proportion (0.5)of severe cases out of all cases
...
# 52.1% outpatient
# we take conservative estimates because another study states that 5%-10% of # symptomatic infections become severe cases
xbar2 <- inc_rate / 0.419 
``` 
...
overall_incid_rate_cholera[country == "Bangladesh", ir_sim := xbar]
overall_incid_rate_cholera[country == "Bangladesh", ir_sim_sd := xbar*1.5]
...
#### India
# [Deen et al](https://journals.plos.org/plosntds/article?id=10.1371/journal.pntd.0000173) showed that the incidence rate from Kolkata, India (1 May 2003 to 30 April 2005) was $1.6$ per $1000$ pyo

```{r}
xbar <- 1.6/1000 * person_years

overall_incid_rate_cholera[country == "India", ir_sim := xbar]
overall_incid_rate_cholera[country == "India", ir_sim_sd := xbar*1.5]
```
...
### 
We used the raster-based incidence rate where applicable and used cases reported to WHO otherwise. As we do not have full posterior distribution for the raster-based incidence rate, we a simplified lognormal distribution where standard
...
available, WHO case reported data are used instead. Incidence rates are realized as a Lognormal random variable whose mean equals the mean (
...
Lancet (2018) article. It may be worth considering varying the standard deviation across the range (e.g., 0.2-3)
...
usethis::use_data(cfr_data_cholera, overwrite = TRUE)


# -------------------------------------------------------------------------
# Edit 12 Jan 2024
# Cholera case data were download https://worldhealthorg.shinyapps.io/page10cholera_data/
d <- fread("inst/extdata/Cholera_cases_2000_2022.csv")
d$Country <- clean_country_names(d$Country)
# d$CFR == round(100*d$Deaths/d$Total_Cases, digits=2)
d %>% filter(Year > 1999) %>% 
  group_by(Country) %>% 
  summarize(sum_case = sum(Total_Cases, na.rm=T),
            sum_death = sum(Deaths, na.rm=T),
            sample_size = n()) -> d1

d1 |> mutate(estimate = sum_death / sum_case,
            se = sqrt(estimate*(1-estimate)/sample_size),
            lower = ifelse(estimate - 1.96*se > 0, estimate - 1.96*se, 0),
            upper = estimate + 1.96*se) -> d2

# d2$estimate <- 100*d2$estimate
# d2$se <- 100*d2$se
# d2$lower <- 100*d2$lower
# d2$upper <- 100*d2$upper

cntry_df <- data.frame(Country = target_countries_cholera)
df <- dplyr::left_join(cntry_df, d2, by="Country")

# data are missing for a few countries
# values for the following four countires are either unavailable or zero (Bangladesh)
# Bangladesh
# Central African Republic
# Congo, Democratic Republic of the
# Algeria
cntries = c("Bangladesh","Central African Republic",
                     "Congo, Democratic Republic of the","Algeria")
for (cn in cntries) {
  df[df$Country == cn, ]$estimate = 
    cfr_data_cholera[cfr_data_cholera$country == cn,]$mean / 100
  df[df$Country == cn, ]$se = 
    cfr_data_cholera[cfr_data_cholera$country == cn,]$se / 100
  df[df$Country == cn, ]$lower = 
    cfr_data_cholera[cfr_data_cholera$country == cn,]$lower / 100
  df[df$Country == cn, ]$upper = 
    cfr_data_cholera[cfr_data_cholera$country == cn,]$upper / 100
}

names(df) = c("country","sum_case","sum_death",
               "sample_size","estimate",
               "se","lower",'upper')

cfr_data_cholera_2023 = setDT(df) # to use data.table functions
usethis::use_data(cfr_data_cholera_2023, overwrite = TRUE)
```
...
We assume that people who have at least basic sanitation are safe from these diseases (or at least, have reduced risk). We used the WaSH data from JMP (2000-2017) to fit a saturating exponential function to roughly estimate the trend of WaSH.
