...
Disease burden and case fatality ratio (CFR)s were reviewed. 

Passive, facility-based surveillance is the primary data source for cholera burden estimates. Within each country, counts of cholera cases move from primary facilities up the administrative ladder before national totals are reported to WHO. The following processes may distort these numbers. 

Incomplete case capture within the health system.
Many symptomatic infections are never reported to health facilities. A meta-analysis of all-cause diarrhoea by Wiens et al. found that only 32.8 % of diarrhoeal cases seek facility-based care on average (95 % CI 28.1 – 37.9 %; prediction interval 3.3 - 87.5 % (1). Because cholera is often more severe than generic diarrhoea, 32.8 % may better serve as an upper bound for mild infections and a lower bound for rapidly dehydrating cases. In Sitakunda, Bangladesh, antibody data showed that out of every 3 280 serologically-defined infections, 4 became symptomatic and only 1 was recorded in a health facility (2). Thus ≈75 % of symptomatic infections were missed even before reporting losses
 
Losses along the reporting chain
Even cases that do reach facilities may never be reported to WHO because of political, economic, or social disincentives—such as concern that acknowledging cholera highlights inadequate water and sanitation or could trigger trade and travel restrictions (3). The magnitude of this attrition is not clear. An early modeling study claimed that true incidence might be ten-fold higher than reported (10).

Misclassification of non-cholera diarrhoea.
Conversely, the non-specific clinical definition inflates counts: only 52 % of suspected cases are confirmed Vibrio cholerae infections on average in Bangladesh (4). This means assuming perfect sensitivity and 56 % specificity, the raw “suspected-case” count overstates true medically-attended cholera by 1 / 0.52 ≈ 1.9-fold. 

Under-ascertainment of deaths
Only partial representation of cholera through passive surveillance is also evident in the number of deaths. Many cholera deaths occur outside of health facilities. A meta-analysis of several outbreaks (3,6–9) shows that the true number of deaths may be a few fold greater than the number of deaths reported in health facilities (Wiens et al. personal communication). In addition, a few studies suggest that the mortality is even more severely underestimated than the case. For example, active case finding during an outbreak in Kenya identified 37% more cases and 200% more deaths than reported, leading to CFR increasing from 5.5% (in passive surveillance) to 11.4% (in active case finding) (3).
Implications for burden estimation.
Passive surveillance probably captures only a small, highly uncertain fraction of true symptomatic infections before additional losses in the reporting chain and misclassification are considered. Deaths may still be under-ascertained, so the true CFR could be up to twice the facility-based estimate.
Assumptions used in our modelling.
To adjust previous burden and oral cholera vaccine (OCV) impact estimates, we assumed that reported figures represent 3.3 %–87.5 % of actual symptomatic cases. We don’t have quantitative evidence on extent to which collected cases are under-reported to WHO because of various reasons including economic reaons. We make an assumption that the degree may cancel over-counting that may arise by using the definition of a suspected case of cholera, which was loosefly affected by the fact that cholera burden may be 10-fold higher than reported. For mortality, we allowed for preferential under-reporting of deaths, permitting the case-fatality ratio to rise by up to a factor of two.
To propagate these uncertainties into vaccine-impact and burden models we adopt the following priors:

1.	Wiens KE, Miller MH, Costello DJ, Solomon AP, Hilbert SM, Shipper AG, et al. Care-seeking for diarrheal illness: A systematic review and meta-analysis. PLOS Global Public Health. 2025 Apr 22;5(4):e0004483.
2.	Hegde ST, Khan AI, Perez-Saez J, Khan II, Hulse JD, Islam MT, et al. Clinical surveillance systems obscure the true cholera infection burden in an endemic region. Nat Med. 2024 Feb 20;
3.	Ganesan D, Gupta SS, Legros D. Cholera surveillance and estimation of burden of cholera. Vaccine. 2020 Feb 29;38:A13–7.
4.	Wiens KE, Xu H, Zou K, Mwaba J, Lessler J, Malembaka EB, et al. Estimating the proportion of clinically suspected cholera cases that are true Vibrio cholerae infections: A systematic review and meta-analysis. PLOS Med. 2023 Sep 14;20(9):e1004286.
5.	Shikanga OT, Mutonga D, Abade M, Amwayi S, Ope M, Limo H, et al. High mortality in a cholera outbreak in western Kenya after post-election violence in 2008. Am J Trop Med Hyg. 2009 Dec;81(6):1085–90.
6.	Routh JA, Loharikar A, Fouché MDB, Cartwright EJ, Roy SL, Ailes E, et al. Rapid assessment of cholera-related deaths, Artibonite Department, Haiti, 2010. Emerg Infect Dis. 2011 Nov;17(11):2139–42.
7.	Page AL, Ciglenecki I, Jasmin ER, Desvignes L, Grandesso F, Polonsky J, et al. Geographic Distribution and Mortality Risk Factors during the Cholera Outbreak in a Rural Region of Haiti, 2010-2011. Ryan ET, editor. PLOS Neglected Tropical Diseases. 2015;9(3):e0003605.
8.	Djouma FN, Ateudjieu J, Ram M, Debes AK, Sack DA. Factors Associated with Fatal Outcomes Following Cholera-Like Syndrome in Far North Region of Cameroon: A Community-Based Survey. Am J Trop Med Hyg. 2016 Dec 7;95(6):1287–91.
9.	McCrickard LS, Massay AE, Narra R, Mghamba J, Mohamed AA, Kishimba RS, et al. Cholera Mortality during Urban Epidemic, Dar es Salaam, Tanzania, August 16, 2015–January 16, 20161. Emerg Infect Dis. 2017 Dec;23(Suppl 1):S154–7.
10.	Ali M, Nelson AR, Lopez AL, Sack DA. Updated Global Burden of Cholera in Endemic Countries. PLoS Negl Trop Dis. 2015 Jun 4;9(6):e0003832.

## Update input parameters

Input parameters may be updated using `usethis` package. For example, if the
parameter named input_params_to_update may be updated as in the following (e.g, `input_params_to_update <- updated_value;usehis::use_data(input_params_to_update, overwrite = T)`)
...
# tst <- "20250623T160631"
# tst <- "20250625T181526" # new runs with CRF unit conversion corrected
# tst <- "20250703T234602"
target_countries_cholera_ <- target_countries_cholera
# just to simulate Bangladesh
target_countries_cholera <- target_countries_cholera[6]

params_list <- vector("list", length(target_countries_cholera))
...
# EDIT 2025
# Incidence rate estimates for Bangladesh is based on seroincidence and 
# already accounts for the healthcare seeking probability 
params_list$Bangladesh$care_seeking_prop <- 1
...
cohort_list <- readRDS(paste0("outputs/cohort_list_Cholera_", tst, ".rds"))
params_list <- readRDS(paste0("outputs/params_Cholera_", tst, ".rds"))
...
# cntry_id0 <- 31
# cntry_id1 <- length(target_countries_cholera)
# id <- 2
for (id in 1:3) {
# for (id in seq_along(vacc_scenarios_cholera)) {
  ...
  vc_data <- NULL
  if (id > 1) {
    vc_data <- vacc_cov_input_cholera[[id-1]] # length = 2
  }  
  ...
  sim <- simulate_cholera(disease = dis, 
                          year = "2000:2100",
                          # country = target_countries_cholera[cntry_id0:cntry_id1],
                          country = target_countries_cholera,
                          params_list = params_list,
                          vacc_scenario = vacc_scenario,
                          vacc_cov_data = vc_data,
  ...
  saveRDS(sim$central, paste0("outputs/central_output_list_", 
                            tst, "_", dis, "_", vacc_scenario, ".rds"))
  # 
  # saveRDS(sim$stoch, paste0("outputs/stoch_output_list_cntry_", cntry_id0, "_",
  #                           cntry_id1, "_", 
  #                           tst, "_", dis, "_", vacc_scenario, ".rds"))
  # saveRDS(sim$central, paste0("outputs/central_output_list_cntry_", cntry_id0, "_",
  #                           cntry_id1, "_", 
  #                           tst, "_", dis, "_", vacc_scenario, ".rds"))
}
```
...
`simulate_cholera` function returns a list 
A data.frame is produced using `do.call(rbind, x)` or `data.table::rbindlist`.
...
# id0 <- 31
# id1 <- 47
for (stocen in c("central", "stoch")){
  for (vacc in vacc_scenarios_cholera) {
    
    res_list <- readRDS(paste0("outputs/", stocen, "_output_list_", tst,
                     "_", dis, "_", vacc, ".rds"))
    # res_list <- readRDS(paste0("outputs/", stocen, "_output_list_cntry_", id0,"_", 
    #                            id1, "_", tst, "_", dis, "_", vacc, ".rds"))
    res <- rbindlist(res_list)
    
    saveRDS(res, paste0("outputs/", stocen, "_", dis, "_",
                                    vacc, "_", tst, ".rds"))
  
    # saveRDS(res, paste0("outputs/", stocen, "_cntry_", id0,"_", 
    #                            id1, "_", dis, "_", 
    #                                 vacc, "_", tst, ".rds"))
  }
}
...
for (stocen in c("central", "stoch")){
  for (vacc in vacc_scenarios_cholera) {
    fls <- list.files(path="outputs/", pattern = paste0(stocen, ".*_", vacc, "_.*", tst, "\\.rds"), 
                      full.names = T)
    lst <- vector('list', length(fls))
    for (i in 1:length(fls)) {
      lst[[i]] <- read_rds(fls[i])
    }
    
    res <- rbindlist(lst)
    saveRDS(res, paste0("outputs/", stocen, "_BGD_", dis, "_", vacc, "_", tst, ".rds"))
  }
}
```
...
saveRDS(res, paste0("outputs/params_BGD_", dis, "_", tst, ".rds"))
```
...
for (stocen in c("central", "stoch")) {
  for (vacc in vacc_scenarios_cholera) {
    d0 <- read_rds(paste0("outputs/", stocen, "_Cholera_", vacc, "_20250625T181526.rds"))
    message(length(unique(d0$country)))
    d1 <- read_rds(paste0("outputs/", stocen, "_BGD_Cholera_", vacc, "_20250704T114601.rds"))
    message(length(unique(d1$country)))
    # d0 %>% filter(age == 0 & year ==2000) %>% count(country)
    # nrow(d0) == nrow(d1)*47
    # d0_ <- d0
    d0 %>% filter(country != "BGD") -> d0_
    d0_new <- rbind(d0_, d1)
    # d0 %>% filter(age == 0 & year ==2000) %>% group_by(country) %>% summarise(cohort_size)
    # d0_new %>% filter(age == 0 & year ==2000) %>% group_by(country) %>% summarise(cohort_size)
  
    # check if they are correct
    d0 %>% filter(age == 0 & year ==2000) %>% group_by(country) %>% summarise(cases)
    # d0_new %>% filter(age == 0 & year ==2000) %>% group_by(country) %>% summarise(cases)
    # d1 %>% filter(age == 0 & year ==2000) %>% group_by(country) %>% summarise(cases)
  
    saveRDS(d0_new, paste0("outputs/", stocen, "_Cholera_", vacc, "_", tst, ".rds"))
  }
}


d0 <- read_rds("outputs/params_Cholera_20250625T181526.rds")
d1 <- read_rds("outputs/params_BGD_Cholera_20250704T114601.rds")
d0 %>% filter(country != "Bangladesh") -> d0_
d0_new <- rbind(d0_, d1)


d0 %>% filter(country == "Bangladesh") %>% summarize(case_fatality_ratio)
d0_new %>% filter(country == "Bangladesh") %>% summarize(case_fatality_ratio)

d0 %>% filter(country == "Bangladesh") %>% summarize(care_seeking_prop)
d0_new %>% filter(country == "Bangladesh") %>% summarize(care_seeking_prop)

saveRDS(d0_new, paste0("outputs/params_Cholera_", tst, ".rds"))
fwrite(d0_new, paste0("outputs/params_Cholera_", tst, ".csv"))
```

Check the saved files
```{r}
# for (stocen in c("central", "stoch")){
for (stocen in c("central")){  
  for (vacc in vacc_scenarios_cholera) {
    cat("stocen:", stocen, ", vacc:", vacc, "\n")
    fls <- list.files(path="outputs/", pattern = paste0(stocen, ".*_", vacc, "_.*", tst, "\\.rds"), 
                      full.names = T)
    fl <- read_rds(fls[1])
    fwrite(fl, paste0("outputs/", stocen, "_Cholera_", vacc, "_", tst, ".csv"))
    message(paste0("nrow: ", fl %>% filter(age == 0 & year ==2000) %>% nrow())) 
    message(paste0("unique countries: ",fl %>% filter(age == 0 & year ==2000) %>% pull(country) %>% unique() %>% length())) 
  }
}
```
...
for (cntry in target_countries_cholera) {
  ...
  sim %>%
      ...
      summarize(cohort_size = sum(cohort_size),
                ...
                deaths = sum(deaths),
                yll = sum(yll)) -> df
  cat("cohort size:", format(df$cohort_size, big.mark=","),  
      "; cases:", format(df$cases, big.mark=","),
      "; DALYs:", format(df$dalys, big.mark=","), 
      "; YLLs:", format(df$yll, big.mark=","), "\n")
...
# path_to_folder <- "G:\\My Drive\\Projects\\VIMC\\cholera_typhoid_vimc_ivi\\outputs\\"
path_to_folder <- "outputs/"
tst1 <- "20250704T130601"
tst2 <- "20250625T181526"
vacc <- "novacc"
# vacc <- "campaign_default_ocv1" 
# vacc <- "campaign_default_ocv12"
sim1 <- readRDS(paste0(path_to_folder, "central_", dis, "_", vacc, "_", tst1, ".rds"))
sim2 <- readRDS(paste0(path_to_folder, "central_", dis, "_", vacc, "_", tst2, ".rds"))
...
for (cntry in target_countries_cholera) {
  ...
  sim1 %>%
      ...
      summarize(cohort_size = sum(cohort_size),
                ...
                deaths = sum(deaths),
                yll = sum(yll)) -> df1
  cat("NEW cohort size:", format(df1$cohort_size, big.mark=","),  
      "; cases:", format(df1$cases, big.mark=","),
      "; DALYs:", format(df1$dalys, big.mark=","), 
      "; YLLs:", format(df1$yll, big.mark=","), "\n")
  sim2 %>%
      filter(country_name == cntry) %>%
      # group_by(year) %>%
      summarize(cohort_size = sum(cohort_size),
  ...
  cat("OLD cohort size:", format(df2$cohort_size, big.mark=","),  
      "; cases:", format(df2$cases, big.mark=","),
      "; DALYs:", format(df2$dalys, big.mark=","), 
      "; YLLs:", format(df2$yll, big.mark=","), "\n")
  invisible(readline(prompt = "Press RETURN to continue")) }
```

# Summaries by age  (VIMC first)
```{r}
tst <- "20250704T130601"
vacc <- "novacc"
# vacc <- "campaign_default_ocv1" 
# vacc <- "campaign_default_ocv12"
sim <- readRDS(paste0("outputs/central_Cholera_", vacc, "_", tst, ".rds"))
sim %>% group_by(year) %>% 
...
ggplot(df)+
  geom_line(aes(year, deaths_sum))
ggplot(df)+
  geom_line(aes(year, cases_sum))

sim %>% group_by(year, age) %>% 
  summarize(deaths_sum = sum(deaths),
            cases_sum = sum(cases), 
            dalys_sum = sum(dalys),
            yll_sum = sum(yll)) -> df

ggplot(df, aes(year, deaths_sum, fill=as.factor(age))) +
...
ggplot(df, aes(x = factor(year), y = cases_sum, fill = age)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(
    title = "Distribution of Cases by Year and Age Group",
    x = "Year",
    y = "Total Cases",
    fill = "Age"
  ) +
  ...
  scale_x_discrete(
    breaks = function(x) x[seq(1, length(x), 10)]
  )+
